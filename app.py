...
def initialize_sample_data():
    """Initialize the local database with sample data for testing"""
    if not local_db['users']:
        # Sample users
        local_db['users'] = {
            'drsmith@example.com': {
                'user_id': 'doc-001',
                ...
                'about': 'Dr. Smith is a board-certified cardiologist with 15 years of experience.'
            },
            'drjohnson@example.com': {
                ...
                'is_active': True,
                'patients': set(),
                'specialties': ['Diabetes Management', 'Thyroid Disorders'],
                'languages': ['English', 'French'],
                'about': 'Dr. Johnson specializes in diabetes and thyroid disorders.'
            },
            'saikiran@gmail.com': {
                'user_id': 'doc-003',
                'name': 'Dr. Sai',
                ...
                'about': 'Dr. Sai is a general practitioner with expertise in preventive medicine.'
            },
            'patient@example.com': {
                'user_id': 'pat-001',
                'name': 'John Doe',
...
@app.route('/logout')
def logout_page():
    """Simple logout page that redirects to home"""
    session.clear()
...
def book_appointment():
    if request.method == 'POST':
        ...
        notification = {
            ...
            'message': f'New appointment request from {patient_name} for {date} at {time}',
            'timestamp': datetime.now().isoformat(),
            'read': False,
            'type': 'appointment_request'
        }
        local_db['notifications'].append(notification)
    ...
    return render_template('book_appoinment.html', user_name=session['name'], doctors=doctors)
...
def create_prescription():
    ...
    for i, med in enumerate(medications):
        if med.strip():  # Only add non-empty medications
    ...
    notification = {
        ...
        'message': f'New prescription available for {diagnosis}',
        'timestamp': datetime.now().isoformat(),
        'read': False,
        'type': 'prescription'
    }
...
def update_appointment_status():
    ...
    for appointment in local_db['appointments']:
        if appointment.get('id') == appointment_id:
            ...
            notification = {
                'id': f"notif-{str(uuid.uuid4())[:8]}",
                'user_email': appointment['patient'],
...
def search():
    ...
    results = []
    for email, user in local_db['users'].items():
        if (query.lower() in user.get('name', '').lower() or 
            query.lower() in user.get('specialization', '').lower() or
...
def view_appointment(appointment_id):
    ...
    if user_role == 'patient' and appointment.get('patient') != user_email:
        flash('Access denied.', 'danger')
        return redirect(url_for('index'))
    
    if user_role == 'doctor' and appointment.get('doctor') != user_email:
        flash('Access denied.', 'danger')
        return redirect(url_for('index'))
...
def get_appointments():
    ...
    if user_role == 'patient':
        appointments = [apt for apt in local_db['appointments'] if apt.get('patient') == user_email]
    else:  # doctor
        appointments = [apt for apt in local_db['appointments'] if apt.get('doctor') == user_email]
...
def get_notifications():
    ...
    notifications = local_db['notifications']
    user_notifications = [n for n in notifications if n.get('user_email') == user_email]
...
def mark_notification_read(notification_id):
    ...
    for notification in local_db['notifications']:
...
def update_profile():
    ...
    if user.get('role') == 'doctor':
        user['specialization'] = request.form.get('specialization', user.get('specialization'))
        user['experience'] = int(request.form.get('experience', user.get('experience', 0)))
        user['hospital'] = request.form.get('hospital', user.get('hospital'))
